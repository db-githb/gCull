...
def raytracing(
    ...
    """Render a set of 3D Gaussians (N) to a batch of image planes (C) using raytracing as in Gaussian Opacity Fields.

    TODO: update the doc to raytracing.
    ...
    # we don't need to gradient for the projection, it only used for creating tile-based rendering
    with torch.no_grad():
    ...
    if colors.shape[-1] > channel_chunk:
        ...
        for i in range(n_chunks):
            ...
            render_colors_, render_alphas_ = raytracing_to_pixels(
                means2d,
                conics,
                colors_chunk,
                opacities,
        ...
        render_colors, render_alphas = raytracing_to_pixels(
            means2d,
            conics,
            colors,
            opacities,
    ...
    return render_colors.squeeze(0)[:,:,:3], render_alphas, meta