...
class BaseRender:
    ...
    filter3D: bool = False
    renderImg: bool = False
    two_pass: bool = False
    filter3D_scale: float = 0.2
    d_thresh: float = 1000
    xg_thresh: float = 0.5
    m_thresh: float = 0.5
    s_thresh: float = 0.5
...
class RenderCameraPath(BaseRender):
    ...
    def main(self) -> None:
        ...
        pipeline.model.filter3D = self.filter3D
        pipeline.model.renderImg = self.renderImg
        pipeline.model.two_pass = self.two_pass
        pipeline.model.filter3D_scale = self.filter3D_scale
        pipeline.model.d_thresh = self.d_thresh
        pipeline.model.xg_thresh = self.xg_thresh
        pipeline.model.m_thresh = self.m_thresh
        pipeline.model.s_thresh = self.s_thresh
        pipeline.model.cc_thresh = self.cc_thresh
