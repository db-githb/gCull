            ...
            for camera_idx in progress.track(range(cameras.size), description=""):
                ...
                if crop_data is not None:
                    ...
                    with torch.no_grad():
                        ...
                        temp = get_rasterizer_output(pipeline.model, cam, True)
                        outputs = {"rgb": sort_package(temp, cam)["rgb"]}
                ...
                # Add closest training image to the right of the rendered image -DOES NOT GO IN HERE
                if render_nearest_camera:
                    assert train_dataset is not None
                    assert train_cameras is not None
