...
class SplatfactoModel(Model):
    ...
    def renderer_rgb(self, camera: Cameras) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        # means3D =  torch.ones_like(self.means) 
        # opacities = torch.sigmoid(torch.ones_like(self.opacities))
        # scales =  torch.ones_like(self.scales) * .01
        # rot_quats = torch.tensor([0.0,0.0,0.0,1.0], device="cuda").repeat(self.means.shape[0], 1, 1)
        # features_dc_crop = torch.zeros_like(self.features_dc)
        # features_rest_crop = torch.zeros_like(self.features_rest)
            
        means3D = self.means
        opacities = torch.sigmoid(self.opacities)
        scales = torch.exp(self.scales)
        rot_quats = self.quats / self.quats.norm(dim=-1, keepdim=True)
        ...
        # rgbs = features_dc_crop
        if self.config.sh_degree > 0:
            viewdirs = means3D.detach() - optimized_camera_to_world.detach()[:3, 3]  # (N, 3)
        ...
        rgb, alpha = rasterize_splats(  # type: ignore
            self.xys,
            depths,
            self.radii,
            conics,
        ...
        if self.config.output_depth_during_training or not self.training:
            depth_im = rasterize_splats(  # type: ignore
                self.xys,
                depths,
                self.radii,
                conics,
