...
def run_colmap(
    ...
    model_orientation_aligner_cmd = " ".join(model_orientation_aligner_cmd)

    with status(
        msg="[bold yellow]Running COLMAP model aligner...",
        spinner="circle",
        verbose=verbose,
    ):
        run_command(model_orientation_aligner_cmd, verbose=verbose)
    CONSOLE.log("[bold green]:tada: Done COLMAP model aligner.")
