...
class ControlPanel:
    ...
    def __init__(
        ...
        self._filter3D = ViewerCheckbox(
            "Enable",
            False,
            cb_hook=lambda han: [self.update_control_panel(), rerender_cb(han)],
            hint="Render with 3D smoothing filter",
        )
        self._renderImg = ViewerCheckbox(
            "Enable",
            False,
            cb_hook=lambda han: [self.update_control_panel(), rerender_cb(han)],
            hint="Render Image",
        ...
        self._two_pass = ViewerCheckbox(
            ...
            False,
            cb_hook=lambda han: [self.update_control_panel(), rerender_cb(han)],
            hint="Render two pass filter",
        )
        self._clamp_alpha = ViewerCheckbox(
            "Enable",
            False,
            cb_hook=lambda han: [self.update_control_panel(), rerender_cb(han)],
            hint="Render alpha min value clamped",
        )
