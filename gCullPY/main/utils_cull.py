...
def get_cull_list(model, camera, bool_mask):
    ...
    filter_3D = compute_3D_filter(model, camera,.2) #model.filter3D_scale, renderer) # compute_3D_filter per camera (GOF does it for each camera at the beginning of training)
