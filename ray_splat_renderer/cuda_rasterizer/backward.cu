...
__device__ glm::mat4 computeGaussian2World_backward(const float3& mean, const glm::vec4 rot){
	...
	float* __restrict__ dL_doffset)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		{	
			...
			// MINE
			float3 means3D_j = collected_means3d[j];
			glm::vec4 rot_j = collected_rot[j];
			// TODO treat G2W as view2gaussian and pass it in
			// transform point of interesection from local guassian space to world space
			glm::mat4 G2W = computeGaussian2World_backward(means3D_j, rot_j);

			// UNDOING EQ.2 from GOF paper
			float x_w[3] = {
				(G2W[0][0] * x_g.x + G2W[1][0] * x_g.y + G2W[2][0] * x_g.z) + G2W[3][0],
				(G2W[0][1] * x_g.x + G2W[1][1] * x_g.y + G2W[2][1] * x_g.z) + G2W[3][1],
				(G2W[0][2] * x_g.x + G2W[1][2] * x_g.y + G2W[2][2] * x_g.z) + G2W[3][2]
			};

			int insert = -1;

			if (insert == -1){
				// save top x contributers
				for(int i = 0; i < NUM_CONTRIBUTORS; i++){
					// >= to ensure backwards pass find gaussian selected by forward (in cases when multiple gaussians have same power)
			...
			if (insert > -1){
				// set power
				for(int i = NUM_CONTRIBUTORS-1; i > insert; i--){
					max_power[i] = max_power[i-1];
					for(int ch = 0; ch<C; ch++){
					dL_doffset[pix_id*NUM_CONTRIBUTORS*3+i*3+ch] = dL_doffset[pix_id*NUM_CONTRIBUTORS*3+(i-1)*3+ch];
				}
				}
				max_power[insert] = power;
				for(int ch = 0; ch<C; ch++){
					dL_doffset[pix_id*NUM_CONTRIBUTORS*3+insert*3+ch] = x_w[ch];
				}
			}
